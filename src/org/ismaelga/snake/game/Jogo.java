...
/**
 * Classe principal do projecto Snake I.
 * É esta classe que fornece todo o interface e controla todas as outras classes.
 * @Ismael
 * @04-2010
 */
public class Jogo  implements java.io.Serializable {
     ...
     * @param largura Largura do campo de jogo ou terreno. Tem de ser maior que 20.
     * @param altura Altura do campo de jogo ou terreno. Tem de ser maior que 20.
     * @param nome Nome do jogador.
     */
    ...
    public void mandaActuar() {
        ...
        //estadoJogo();
        //view.showStatus(passo, terreno);
    }
    
    /**
     * Move a cobra N vezes em direcção ao lado para que está virada
     * @param vezes Vezes para a cobra mover.
     */
    ...
    private void limpaMortos() {
        //Percorre os actores e actualiza o campo e a lista de actores retirando os mortos
       Actor actor;
       List<Actor> actoresVivos = new ArrayList<Actor>();
       terreno.clear();
       for(int i=0; i < actores.size(); i++) {
           actor = (Actor)actores.get(i);
           if(actor.isAlive()) {
    ...
    private void criaNovoOvo() {
        ...
        do {
            pos = new Posicao(rand.nextInt(terreno.getAltura()),
                              rand.nextInt(terreno.getLargura()));
        ...
        if(cobraHumana.isAlive()) {
            ...
            for(int i=0; i< actores.size(); i++) {
                ...
                if(actor instanceof Ovo) {
                    ...
                    if(actor instanceof Escaravelho)
    ...
    private void estadoJogo() {
        ...
        System.out.println(separador);
        System.out.println("Jogador: " + nomeJogador + "   nº de jogadas: " + passo + "   pontos: " + cobraHumana.getPontos()+ "   nivel: " + getNivel());
        System.out.println(separador);
        System.out.println("Cobra Utilizador -> VIDAS: "+ cobraHumana.getVidas() +" OVOS: "+cobraHumana.getOvosComidos()+" LADO VIRADA: "+ cobraHumana.getLadoVirada());
        System.out.println("Cobra Computador -> VIDAS: "+ cobraAuto.getVidas() +" OVOS: "+cobraAuto.getOvosComidos()+" LADO VIRADA: "+ cobraAuto.getLadoVirada());
        System.out.println(separador);
    }
    
    public String getEstadoJogo() {
        return "Jogador: " + nomeJogador + "   nº de jogadas: " + passo + "   pontos: " + cobraHumana.getPontos()+ "   nivel: " + getNivel();
    }
    ...
    public void recomecar() {
        ...
        //    System.out.println();
        //}
        //System.out.println("JOGO CRIADO! FAÇA CONTINUAR PARA COMEÇAR O JOGO!");
        //posicoes();
        //estadoJogo();
   ...
   // {
   //     String nomeFicheiro= "ultimoGuardado";
    ...
    public void carregaJogo() throws IOException
    {
        ...
        try{
            ...
            actores = (List)in.readObject();
            passo = in.readInt();
            nomeJogador = (String)in.readObject();
        ...
        for(int i=0;i < actores.size(); i++)
        {
            if(actores.get(i) instanceof Ovo)
                ovo = (Ovo)actores.get(i);
            if(actores.get(i) instanceof Cobra_hum)
                cobraHumana = (Cobra_hum)actores.get(i);
            if(actores.get(i) instanceof Cobra_auto)
                cobraAuto = (Cobra_auto)actores.get(i);
        }
    }
    */
    /*
    ...
    *{
    *    timer = new Timer();
    *    timer.schedule(new TempTask(), tempoEntrePassos - (TEMPO_AUMENTADO_POR_NIVEL * (passo / PASSOS_POR_NIVEL)));
    *}
